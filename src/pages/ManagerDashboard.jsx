...
function ManagerDashboard({ leaveRequests, isLoading, onUpdateStatus, actionLoading }) {
  ...
  const tabCount = (tab) =>
    tab === "All" ? leaveRequests.length : leaveRequests.filter((r) => r.status === tab).length;

  const openModal  = (type, request) => setModal({ open: true, type, request });
  const closeModal = () => {
    if (!isConfirming) setModal({ open: false, type: null, request: null });
  };

  const handleConfirm = async () => {
    setIsConfirming(true);
    try {
      if (modal.type === "approve") await onUpdateStatus(modal.request.id, "Approved");
      if (modal.type === "reject")  await onUpdateStatus(modal.request.id, "Rejected");
    } finally {
  ...
  const isRowBusy = (id) => actionLoading === `status-${id}`;
    ...
    <section className="page">
      ...
      <div className="cards-grid">
        ...
        <SummaryCard title="Pending"  value={stats.pending}
          subtitle="Awaiting your action" icon="⏳" variant="pending"/>
        <SummaryCard title="Approved" value={stats.approved}
          subtitle="Approved this period" icon="✅" variant="approved"/>
      ...
      <div className="table-card" style={{ overflow: "visible" }}>
        ...
        {filteredRequests.length === 0 ? (
          <div className="empty-state">
            <div className="empty-state-icon">{activeFilter === "Pending" ? "✅" : "📭"}</div>
            <h4>{activeFilter === "Pending" ? "All caught up!" : `No ${activeFilter.toLowerCase()} requests`}</h4>
            <p>{activeFilter === "Pending"
              ? "There are no pending requests awaiting your review."
              : "Try selecting a different filter above."}</p>
          </div>
        ) : (
          <>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>End Date</th><th>Duration</th><th>Reason</th>
                    <th>Submitted</th><th>Status</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredRequests.map((req) => (
                    <tr key={req.id} className={isRowBusy(req.id) ? "row--loading" : ""}>
                      <td>
                        <div className="employee-cell">
                          <div className="emp-avatar emp-avatar--manager">{initials(req.employeeName)}</div>
              ...
              {filteredRequests.map((req) => (
                <div key={req.id} className="mobile-card">
                  ...
                  <div className="mc-meta">
                    ...
                    <div className="mc-meta-item"><label>Duration</label><span>{calcDays(req.startDate, req.endDate)} days</span></div>
